...
{
    executableName = "YcyzClass.Desktop.exe";
} else if (OperatingSystem.IsLinux())
{
    executableName = "YcyzClass.Desktop";
}
else
{
    ShowError("YcyzClass 正在不支持的平台上运行，无法继续。");
    return 1;
}
...
// 自动添加可执行权限，防止出现无法运行的问题 https://github.com/YcyzClass/YcyzClass/issues/1212
#if Platforms_Linux
try
{
...
startInfo.EnvironmentVariables["YcyzClass_PackageRoot"] = root;  // 防止因环境变量已设置导致启动器崩溃
...
void ShowError(string message)
{
    ...
    PInvoke.MessageBox(HWND.Null, message,
        "YcyzClass", MESSAGEBOX_STYLE.MB_APPLMODAL | MESSAGEBOX_STYLE.MB_ICONSTOP);
#else
