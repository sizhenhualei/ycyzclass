...
using YcyzClass.Shared.Models.Management;
...
using YcyzClass.Shared.Protobuf.Client;
using YcyzClass.Shared.Protobuf.Service;
using FluentAvalonia.UI.Controls;
using Microsoft.Extensions.Logging;

using static YcyzClass.Shared.Helpers.ConfigureFileHelper;

namespace YcyzClass.Services.Management;
        ...
        try
        {
            ...
            if (Manifest.CoreVersion.Major != IAppHost.CoreVersion.Major)
            {
                await CommonTaskDialogs.ShowDialog("集控核心版本不兼容",
                    $"您正在核心版本主版本号（{IAppHost.CoreVersion}）与集控数据核心版本主版本号（{Manifest.CoreVersion}）不同的 YcyzClass 加入集控，这可能会导致您的数据损坏。请运行兼容贵组织集控服务的 YcyzClass 版本，或者联系您的管理员了解详细信息。");
                return false;
            }
