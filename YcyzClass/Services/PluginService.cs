...
using YcyzClass.Models.Plugins;
    ...
    public static void InitializePlugins(HostBuilderContext context, IServiceCollection services)
    {
        ...
        foreach (var pluginDir in pluginDirs)
        {
            ...
            if (info.IsEnabled && Version.TryParse(info.Manifest.ApiVersion, out var apiVersion) && apiVersion < new Version(2, 0, 0, 0))
            {
                ...
                info.Exception = new InvalidOperationException($"不兼容的 API 版本 {apiVersion}。插件的 API 版本需要至少为 2.0.0.0 才能被当前版本的 YcyzClass 加载。");
                PluginLoadedStatus.Add(info);
            }
        }
        ...
        var suppressMacOsPluginLoadBehavior =
            Environment.GetEnvironmentVariable("YcyzClass_DebugSuppressMacOSPluginLoadBehavior") == "1";
        // 加载插件
