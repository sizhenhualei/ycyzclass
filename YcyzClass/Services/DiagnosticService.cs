    ...
    AppLogService appLogService)
{
    ...
    public async Task ExportDiagnosticData(string path)
    {
        try
        {
            var temp = Directory.CreateTempSubdirectory("YcyzClassDiagnosticExport").FullName;
    ...
    public static void ProcessCriticalException(Exception? ex)
    {
        ...
        var pluginsWarning = Environment.NewLine + Environment.NewLine +
                             "此问题可能由以下插件引起，请在向 YcyzClass 开发者反馈问题前先向以下插件的开发者反馈此问题：" + Environment.NewLine
                             + string.Join(Environment.NewLine,
                                 plugins.Select(x => $"- {x.Manifest.Name} [{x.Manifest.Id}]"));
        var message = $"""
                       很抱歉，YcyzClass 遇到了无法解决的问题，即将退出。堆栈跟踪信息已复制到剪贴板。点击【确定】将退出应用，点击【取消】将启动调试器。
        ...
        // TODO: 实现对话框
        // var r = MessageBox.Show(message, "YcyzClass", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
