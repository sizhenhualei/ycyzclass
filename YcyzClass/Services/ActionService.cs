...
using static YcyzClass.Core.Abstractions.Services.IActionService;
...
public class ActionService : IActionService
{
    ...
    {
        // 1.7.107.0
        // 迁移应用设置行动 ycyzclass.settings
        {
            const string prefix = "ycyzclass.settings.";
            if (actionItem.Id.StartsWith(prefix))
            {
                ...
                actionItem.Id = "ycyzclass.settings";
    ...
    public ActionService(ILogger<ActionService> logger)
    {
        ...
        ActionMenuTree.Add(
            ...
            new ActionMenuTreeItem<RunActionSettings>("ycyzclass.os.run", "应用程序", "\uf4b1",
                s => s.RunType = RunActionSettings.RunActionRunType.Application),
            new ActionMenuTreeItem<RunActionSettings>("ycyzclass.os.run",
                OperatingSystem.IsWindows() ? "cmd 命令" : "终端命令",
                "\ue508",
                s => s.RunType = RunActionSettings.RunActionRunType.Command),
            new ActionMenuTreeItem<RunActionSettings>("ycyzclass.os.run", "文件", "\ue687",
                s => s.RunType = RunActionSettings.RunActionRunType.File),
            new ActionMenuTreeItem<RunActionSettings>("ycyzclass.os.run", "文件夹", "\ue875",
                s => s.RunType = RunActionSettings.RunActionRunType.Folder),
            new ActionMenuTreeItem<RunActionSettings>("ycyzclass.os.run", "Url 链接", "\ue905",
                s => s.RunType = RunActionSettings.RunActionRunType.Url)
        ));

        ActionMenuTree.Add(
            new ActionMenuTreeGroup("提醒", "\ue025",
            new ActionMenuTreeItem<NotificationActionSettings>("ycyzclass.showNotification", "显示提醒…", "\ue02f",
                s => s.IsWaitForCompleteEnabled = false),
            new ActionMenuTreeItem<NotificationActionSettings>("ycyzclass.showNotification", "显示提醒并等待…", "\ue02b",
                s => s.IsWaitForCompleteEnabled = true),
            new ActionMenuTreeItem<WeatherNotificationActionSettings>("ycyzclass.notification.weather", "三天天气预报", "\ue4dc",
                s => s.NotificationKind = 0),
            new ActionMenuTreeItem<WeatherNotificationActionSettings>("ycyzclass.notification.weather", "气象预警", "\uf431",
                s => s.NotificationKind = 1),
            new ActionMenuTreeItem<WeatherNotificationActionSettings>("ycyzclass.notification.weather", "逐小时天气预报", "\uf357",
                s => s.NotificationKind = 2)
        ));

        ActionMenuTree.Add(
            new ActionMenuTreeGroup("YcyzClass", "\ue454",
            new ActionMenuTreeItem("ycyzclass.app.quit", "退出 YcyzClass", "\ue0df"),
            new ActionMenuTreeItem("ycyzclass.app.restart", "重启 YcyzClass", "\ue0bd")
        ));

        return;

        static ActionMenuTreeItem<ModifyAppSettingsActionSettings> SettingItem(string displayName, string icon, string name) =>
            new("ycyzclass.settings", displayName, icon, s => s.Name = name);
    }

    [Obsolete("注意！行动 v2 注册方法已过时，请参阅 YcyzClass 开发文档进行迁移。")]
