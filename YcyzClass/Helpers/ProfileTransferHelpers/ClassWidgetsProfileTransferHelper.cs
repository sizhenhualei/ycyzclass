...
{
    public static Profile ConvertClassWidgets1ProfileToYcyzClassProfile(string path, Profile? profile = null)
    {
        var profileCw = ConfigureFileHelper.LoadConfigUnWrapped<CwProfile>(path);
        var templateJson = new StreamReader(AssetLoader.Open(new Uri("avares://YcyzClass/Assets/default-subjects.json"))).ReadToEnd();
