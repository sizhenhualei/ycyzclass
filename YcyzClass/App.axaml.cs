...
using YcyzClass.Core.Controls.IconSources;
...
using YcyzClass.Platforms.Abstraction.Enums;
...
public partial class App : AppBase, IAppHost
{
    ...
    private void ActivatePackageType()
    {
        ...
        var packageTypeDir = Path.Combine(Environment.GetEnvironmentVariable("YcyzClass_PackageRoot") ?? exeDir,
            "PackageType");
        var fallbackPackageTypeDir = Path.Combine(exeDir, "../", "PackageType");
        var packagingRoot = Path.GetFullPath(Environment.GetEnvironmentVariable("YcyzClass_PackageRoot") ?? 
                                             Path.Combine(exeDir, "../"));
    ...
    private void ActivateAppDirectories()
    {
        ...
        CommonDirectories.AppRootFolderPath = PackagingType switch
        {
            ...
            "installer" or "deb" or "appImage" or "pkg" or "msix" => Path.GetFullPath(Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "YcyzClass", "Data")),
        ...
        if (File.Exists(Path.Combine(CommonDirectories.AppPackageRoot, "YcyzClass" + PlatformExecutableExtension)))
        {
            ExecutingEntrance =
                Path.Combine(CommonDirectories.AppPackageRoot, "YcyzClass" + PlatformExecutableExtension);
        }
        else
        {
            ExecutingEntrance = Environment.ProcessPath?.Replace(".dll", PlatformExecutableExtension) ?? "";
    ...
    internal async void ProcessUnhandledException(Exception e, bool critical=false)
    {
        ...
        IsCrashed = true;
        var safe = _isCriticalSafeModeEnabled && (!(IAppHost.TryGetService<IWindowRuleService>()?.IsForegroundWindowYcyzClass() ?? false));
        ...
        if (!safe)
        {
            ...
            if (plugins.Count > 0)
            {
                var pluginsWarning = "此问题可能由以下插件引起，请在向 YcyzClass 开发者反馈问题前先向以下插件的开发者反馈此问题："+Environment.NewLine
        ...
        switch (Settings.CriticalSafeModeMethod)
        {
            ...
            case 2:
                ...
                try
                {
                    await PlatformServices.DesktopToastService.ShowToastAsync("崩溃报告", $"YcyzClass 发生了一个无法处理的错误：{e.Message}");
                }
                catch (Exception exception)
                {
    ...
    private async void DesktopLifetimeOnStartup(object? sender, ControlledApplicationLifetimeStartupEventArgs e)
    {
        ...
        {
            await CommonTaskDialogs.ShowDialog("检测到应用正在临时目录下运行", "YcyzClass正在临时目录下运行，应用设置、课表等数据很可能无法保存，或在应用退出后被自动删除。在使用本应用前，请务必将本应用解压到一个适合的位置。");
            Environment.Exit(0);
            return;
        }
        ...
        {
            var r = await CommonTaskDialogs.ShowDialog("检测到正在桌面上运行", "YcyzClass正在桌面上运行，应用设置、课表等数据将会直接存放到桌面上。在使用本应用前，请将本应用移动到一个单独的文件夹中。");
            if (r == (object)true)
            {
                Environment.Exit(0);
                return;
        ...
        {
            await CommonTaskDialogs.ShowDialog("目录权限错误", $"YcyzClass无法写入当前目录：{ex.Message}"+Environment.NewLine+Environment.NewLine+"请将本软件解压到一个合适的位置后再运行。");
            Environment.Exit(0);
            return;
        }
        ...
        if (startupCount >= 5 && ApplicationCommand is { Recovery: false, Quiet: false })
        {
            ...
            {
                Title = "进入恢复模式",
                Content = "YcyzClass 多次启动失败，您需要进入恢复模式以尝试修复 YcyzClass 吗？",
        ...
        IAppHost.Host = Microsoft.Extensions.Hosting.Host
            ...
            .Build();
        AppBase.CurrentLifetime = YcyzClass.Core.Enums.ApplicationLifetime.StartingOffline;
        Logger = GetService<ILogger<App>>();
        Logger.LogInformation("YcyzClass {}", AppVersionLong);
        ...
        if (!string.IsNullOrWhiteSpace(ApplicationCommand.ImportV1))
        {
            ...
            await dtWindow.PerformYcyzClassImport(ApplicationCommand.ImportV1, (ImportEntries)entries);
            return;
        }
        ...
        mw.StartupCompleted += (o, args) =>
        {
            ...
            {
                PlatformServices.DesktopToastService.ShowToastAsync("配置文件损坏", "YcyzClass 部分配置文件已损坏且无法加载，这些配置文件已恢复至默认值。点击此消息以查看详细信息和从过往备份中恢复配置文件。", () => GetService<IUriNavigationService>().NavigateWrapped(new Uri("ycyzclass://app/config-errors")));
            }
            if (Settings.CorruptPluginsDisabledLastSession)
            {
                ...
                {
                    Title = "已自动禁用异常插件",
                    Body = "YcyzClass 已自动禁用导致上次崩溃的插件。您可以在排除问题后前往【应用设置】->【插件】中重新启用这些插件，或在【应用设置】->【基本】中调整是否自动禁用异常插件。",
                    Buttons =
                    {
                        {
                            "打开插件设置",
                            () => GetService<IUriNavigationService>()
                                .NavigateWrapped(new Uri("ycyzclass://app/settings/ycyzclass.plugins"))
                        },
                        {
                            "管理异常插件行为",
                            () => GetService<IUriNavigationService>()
                                .NavigateWrapped(new Uri("ycyzclass://app/settings/general"))
                        }
                    }
                };
            ...
            AppBase.CurrentLifetime = YcyzClass.Core.Enums.ApplicationLifetime.Running;
        ...
        uriNavigationService.HandleAppNavigation("helps", args => uriNavigationService.Navigate(new Uri("https://docs.ycyzclass.tech/app/")));
    ...
    private async Task ProcessInstanceExisted()
    {
        var dialog = new TaskDialog()
        {
            Title = "YcyzClass 已在运行",
            Content = "YcyzClass 已经启动，请通过任务栏托盘图标进行设置等操作。" +Environment.NewLine+Environment.NewLine+
            ...
            [
                new TaskDialogCommand()
                {
                    ...
                    Text = "重启当前实例",
                    Description = "结束正在运行的 YcyzClass 实例，然后再次启动本实例。",
    ...
    {
        if (CurrentLifetime == YcyzClass.Core.Enums.ApplicationLifetime.Stopping)
        {
            return;
        }
        _ = Dispatcher.UIThread.InvokeAsync(() =>
        {
            ...
            CurrentLifetime = YcyzClass.Core.Enums.ApplicationLifetime.Stopping;
