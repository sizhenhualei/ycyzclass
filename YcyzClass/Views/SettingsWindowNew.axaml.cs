...
public partial class SettingsWindowNew : MyWindow, INavigationPageFactory
{
        ...
        catch (Exception ex)
        {
            ...
            if (info.Id != ErrorPageId)
            {
                await CoreNavigate(SettingsWindowRegistryService.Registered.First(x => x.Id == ErrorPageId),
                    new Uri("ycyzclass://app/settings/_error?error=true"));
            }
        }
        //finally
        //{
    ...
    public async void Open()
    {
        ...
        TutorialService.PushToNextSentence("ycyzclass.getStarted.welcome/systems");
    }
    ...
    {
        IAppHost.GetService<IUriNavigationService>().NavigateWrapped(new Uri("ycyzclass://app/settings/management?ci_keepHistory=true"));
    }
