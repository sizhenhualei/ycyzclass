...
/// </summary>
[HidePageTitle]
[SettingsPageInfo("about", "关于 YcyzClass", "\ue9e4", "\ue9e3", SettingsPageCategory.About)]
public partial class AboutSettingsPage : SettingsPageBase
{
    ...
    public AboutSettingsPage()
    {
        DataContext = this;
        InitializeComponent();
        var r = new StreamReader(AssetLoader.Open(new Uri("avares://YcyzClass/Assets/LICENSE.txt")));
        ...
        try
        {
            ViewModel.SettingsService.Settings.ContributorsCache =
                await WebRequestHelper.Default.GetJson<AllContributorsRc>(new Uri(
                    "https://ghfast.top/https://raw.githubusercontent.com/YcyzClass/YcyzClass/master/.all-contributorsrc"));
        }
        catch (Exception ex)
        {
    ...
    private async void Sayings_OnMouseLeftButtonDown(object sender, RoutedEventArgs e)
    {
        ...
        {
            var stream = AssetLoader.Open(new Uri("avares://YcyzClass/Assets/Tellings.txt"));
    ...
    {
        var license = await new StreamReader(AssetLoader.Open(new Uri("avares://YcyzClass/Assets/LICENSE.txt")))
            .ReadToEndAsync();
    ...
    private async void DebugBorder_OnPointerPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        if (ViewModel.AppIconClickCount >= 10)
        {
            ...
            var textBlock = new TextBlock {
                ...
                Text = "您正在发布版本的 YcyzClass 中启用仅供开发使用的调试菜单。请注意此功能仅限于开发和调试用途，YcyzClass 开发者不对以非开发用途使用此页面中功能造成的任何后果负责，也不接受以非开发用途使用时产生的 Bug 的反馈。"+Environment.NewLine
            };
